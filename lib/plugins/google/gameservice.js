...
.defines(function(){
	...
	ig.GGS.unlock = function(achName) {
		if(ig.GGS.constants.ACHIEVEMENTS.hasOwnProperty(achName)) {
			ig.GGS.achManager.unlockAchievement(ig.GGS.constants.ACHIEVEMENTS[achName].code);
		} else {
			console.log('Unknown Acheivement:', achName);
		}
	},

	ig.GGS.progress = function(achName, amount) {
		if(ig.GGS.constants.ACHIEVEMENTS.hasOwnProperty(achName)) {
			ig.GGS.achManager.submitProgress(ig.GGS.constants.ACHIEVEMENTS[achName].code, amount);
		} else {
			console.log('Unknown Acheivement:', achName);
		}
	},

	ig.GGS.submitScore = function(ldbName, score) {
		if(ig.GGS.constants.LEADERBOARDS.hasOwnProperty(ldbName)) {
			ig.GGS.leadManager.gotScore(score, ig.GGS.constants.LEADERBOARDS[ldbName].code);
		} else {
			console.log('Unknown Leaderboard:', ldbName);
		}
	},

	ig.GGS.event = function(action, label, value) {
		if(typeof ga === "undefined") {
			console.log('Looks like we have no GA object');
		} else {
			ga('send', {
				'hitType': 'event',
				'eventCategory': 'ig.GGS',
				'eventAction': action,
				'eventLabel': label,
				'eventValue': value
			});
