...
.defines(function(){
	GoogleGameService = ig.Entity.extend({
		...
		init: function(x, y, settings) {
			...
			ig.GGS.font = new ig.Font('lib/plugins/google/roboto.bold.png');

			this.loginButton = ig.game.spawnEntity(GoogleGameButton, 16, 552,
				{
					face:0,
		...
		update: function() {
			if(ig.GGS.login.loggedIn && this.loginButton) {
				...
				this.achievmentButton = ig.game.spawnEntity(
					GoogleGameButton, 400, 552,
					{
						face: 0,
						text: "Acheivements",
						pressedUp: function() {
							console.log("I'd be showing acheivements if I could");
						}
					}
				)
			}
